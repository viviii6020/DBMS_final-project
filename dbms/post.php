            ...
            <div class="main m0-a">
                <div class="row">
                    <div class="col-md-12 ta-c">
                        <h1 class="fz-30 mt-32 mb-32">最新活動貼文</h1>
                    </div>
                </div>
                ...
                <ul class="row post-list">
                    <?php
                        ...
                        if (!(isset($_SESSION["dbms_project"]))) {
                        $sql = $db->query("SELECT *FROM`addpost` JOIN `member` ON addpost.creater_mId = member.mId ORDER BY pId DESC");
                        }else{ 
                        $mId = $_SESSION['mId'];
                        $sql = $db->query("SELECT *FROM`addpost` JOIN `member` ON addpost.creater_mId = member.mId WHERE addpost.creater_mId!= $mId ORDER BY pId DESC");
                        }
                        $num = $sql->rowCount();
                        $time = 1;
                        foreach ($sql as $row) {
